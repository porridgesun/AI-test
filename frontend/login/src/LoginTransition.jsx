...
const VIDEO_PRELOAD_FALLBACK_MS = 5000;
...
export function LoginTransition() {
  ...
  useEffect(() => {
    if (!requested || readySignal === 0) return undefined;
  ...
  }, [readySignal, requested]);

  useEffect(() => {
    if (readySignal > 0) return undefined;
    const fallback = window.setTimeout(() => setReadySignal((current) => current + 1), VIDEO_PRELOAD_FALLBACK_MS);
  ...
  const visible = requested || phase === "playing" || phase === "frozen";
        ...
        {source && (
          <video
            ...
            onLoadedData={() => {
              ...
              setReadySignal((current) => current + 1);
