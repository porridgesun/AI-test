(function () {
  ...
  function normalizeQuestion(question) {
    ...
    copy.text = copy.text || copy.stem || "";
    copy.options = copy.options || [];
    copy.dimensions = {};
    (copy.dimensionIds || []).forEach(function (id) {
      copy.dimensions[id] = 1;
    });
    return copy;
  }

  function reportToHistory(report) {
    return {
      id: report.id,
      date: String(report.completedAt || "").slice(0, 10),
      title: report.title || "智核觉醒测评报告",
      rating: report.rating || "-",
      stars: Number(report.stars) || 0,
      scores: DIMENSION_KEYS.map(function (key) {
        return Number(report.scores && report.scores[key]) || 0;
      }),
      note: (report.suggestions || []).map(function (item) {
  ...
  function applySession(session) {
    state.api.session = session;
    state.api.currentQuestion = normalizeQuestion(session.currentQuestion);
    state.quiz.currentLevel = session.currentLevel ? session.currentLevel.id : state.quiz.currentLevel;
    state.quiz.started = session.status === "active";
    state.quiz.diff = Number(session.difficulty) || state.quiz.diff;

    var progress = session.levelProgress || [];
    STATIONS.forEach(function (station, index) {
      var item = progress[index] || {};
      var isCurrent = session.currentLevel && session.currentLevel.id === station.id;
      station.score = Number(item.score) || 0;
      station.stars = station.score >= 85 ? 3 : station.score >= 65 ? 2 : station.score > 0 ? 1 : 0;
      station.status = item.completed ? "done" : isCurrent ? "active" : "todo";
      station.pendingTask = null;
    });

    var question = state.api.currentQuestion;
    var currentStation = STATIONS.filter(function (item) {
      return session.currentLevel && item.id === session.currentLevel.id;
    })[0];
    if (currentStation && question && question.type === "practical") currentStation.pendingTask = "practical";
    if (currentStation && question && question.type === "dialogue") currentStation.pendingTask = "dialogue";
  }

  async function refreshHistory() {
    var data = await apiRequest("/api/students/me/history");
    state.history = (data.reports || []).map(reportToHistory);
    state.api.latestReport = data.reports && data.reports[0] ? data.reports[0] : null;
  }

  async function ensureAssessmentSession() {
    if (state.api.session && state.api.session.status === "active") return state.api.session;
    var list = await apiRequest("/api/assessments");
    var assessment = (list.assessments || [])[0];
    if (!assessment) throw new Error("当前暂无可参加的测评");
    var created = await apiRequest("/api/assessments/" + assessment.id + "/sessions", { method: "POST" });
    state.api.assessment = assessment;
  ...
  async function initializeStudentApp() {
    ...
    try {
      var profile = await apiRequest("/api/students/me/profile");
      state.api.profile = profile;
      state.nickname = profile.nickname || profile.name || state.nickname;
      await refreshHistory();
      var current = await apiRequest("/api/sessions/current");
      if (current.session) {
        var list = await apiRequest("/api/assessments");
        state.api.assessment = (list.assessments || []).filter(function (item) {
          return item.id === current.session.assessmentId;
        })[0] || null;
        applySession(current.session);
      } else {
        var list = await apiRequest("/api/assessments");
        state.api.assessment = (list.assessments || [])[0] || null;
      }
      ...
      renderRetestGoal();
      checkResumeSession();
      await loadSelfAssessQuestions();
      switchView("stations");
      if (!state.tutorialDone) setTimeout(openTutorial, 500);
    } catch (error) {
      showToast(error.message || "学生数据加载失败");
    }
  }

  async function loadSelfAssessQuestions() {
    if (SELF_ASSESS.length) return;
    var data = await apiRequest("/api/students/me/self-assessment");
    SELF_ASSESS.push.apply(SELF_ASSESS, data.questions || []);
  }
  ...
  function currentDimensionScores() {
    var reportScores = state.api.latestReport && state.api.latestReport.scores
      ? DIMENSION_KEYS.map(function (key) { return Number(state.api.latestReport.scores[key]) || 0; })
      : [];
    var fallback = reportScores.length ? reportScores : (state.history[0] ? normalizeScores(state.history[0].scores) : []);
  ...
  function saveHistory() {
    /* 历史以 /api/students/me/history 为准，不再写本地演示数据。 */
  }
  ...
  async function startLevel(id) {
    ...
    try {
      await ensureAssessmentSession();
    } catch (error) {
      showToast(error.message || "暂时无法开始测评");
      return;
    }
    var session = state.api.session;
    if (session.status !== "active" || !session.currentQuestion) {
      await finishAssessmentIfNeeded();
      return;
    }
    var currentId = session.currentLevel && session.currentLevel.id;
    if (currentId && currentId !== id && !levelUnlocked(id)) {
      showToast("请按顺序完成当前关卡");
      id = currentId;
    }
    s = STATIONS.filter(function (item) {
      return item.id === id;
    })[0] || currentLevelStation();
    if (state.api.currentQuestion.type === "practical") {
      switchView("practical");
      return;
    }
    if (state.api.currentQuestion.type === "dialogue") {
      switchView("dialogue");
      return;
    }
  ...
  async function beginLevelQuiz() {
    ...
    if (session.currentQuestion.type === "practical") {
      closeModal("quizModal");
      switchView("practical");
      return;
    }
    if (session.currentQuestion.type === "dialogue") {
      closeModal("quizModal");
      switchView("dialogue");
      return;
    }
    ...
    state.quiz.diff = Number(session.difficulty) || 2;
    state.quiz.started = true;
    state.quiz.questionIds = [state.api.currentQuestion.id];
  ...
  async function startObjectiveModule() {
    try {
      await ensureAssessmentSession();
    } catch (error) {
      showToast(error.message || "暂时无法开始测评");
    ...
    if (state.api.session.status !== "active" || !state.api.currentQuestion) {
      await finishAssessmentIfNeeded();
      return;
    }
    var next = STATIONS.filter(function (s) {
    ...
    var currentTask = state.api.currentQuestion.type === "practical" ? "practical" : state.api.currentQuestion.type === "dialogue" ? "dialogue" : null;
    if (currentTask) {
      switchView(currentTask);
      return;
    }
    if (next.pendingTask) {
    ...
    startLevel(state.api.session.currentLevel ? state.api.session.currentLevel.id : next.id);
  }

  async function openDialogueModule() {
    try {
      await ensureAssessmentSession();
    } catch (error) {
      showToast(error.message || "暂时无法进入对话式测评");
      return;
    }
    if (state.api.session.status === "levels_completed") {
      finishQuiz();
      return;
    }
    if (!state.api.currentQuestion || state.api.currentQuestion.type !== "dialogue") {
      showToast("当前测评尚未到达协同空间站对话关卡");
  ...
  async function openPracticalModule() {
    try {
      await ensureAssessmentSession();
    } catch (error) {
      showToast(error.message || "暂时无法进入实操任务");
      return;
    }
    if (state.api.session.status === "levels_completed") {
      finishQuiz();
    ...
    if (!state.api.currentQuestion || state.api.currentQuestion.type !== "practical") {
      showToast("当前测评尚未到达创客工坊实操任务");
      startObjectiveModule();
      return;
    }
    switchView("practical");
  ...
  function switchView(view) {
    ...
    if (view === "dialogue" && state.api.currentQuestion && state.api.currentQuestion.type === "dialogue" && state.chat.round === 0) {
      startChat();
    }
    if (view === "practical") {
      renderTerminalStart();
  ...
  async function openSelfAssess() {
    try {
      ...
      showToast(error.message || "自评题加载失败");
      return;
    }
  ...
  function renderSelfAssess() {
    ...
    Array.prototype.forEach.call(body.querySelectorAll(".self-option"), function (btn) {
      btn.addEventListener("click", function () {
        ...
        var question = SELF_ASSESS[qi];
        state.selfAssess.answers[question.id] = oi;
        ...
        var missing = SELF_ASSESS.some(function (q) {
          return state.selfAssess.answers[q.id] === undefined;
        });
  ...
  async function submitSelfAssess() {
    var answers = {};
    SELF_ASSESS.forEach(function (q) {
      answers[q.id] = state.selfAssess.answers[q.id];
    });
    try {
      var data = await apiRequest("/api/students/me/self-assessment", {
        method: "POST",
      ...
      state.selfAssess.level = data.initialDifficulty <= 1 ? "入门" : data.initialDifficulty >= 5 ? "进阶" : "基础";
      state.quiz.diff = data.initialDifficulty;
    } catch (error) {
      showToast(error.message || "自评提交失败");
      return;
    }
    saveSelfAssess();
  ...
  function levelQuestions() {
    if (state.api.currentQuestion) return [state.api.currentQuestion];
  ...
  function renderQuizSide() {
    var station = currentLevelStation();
    var progress = (state.api.session && state.api.session.levelProgress || []).filter(function (item) {
      return item.levelId === station.id;
    })[0] || {};
    var answered = Number(progress.answerCount) || 0;
    var target = Number(progress.targetCount) || QUIZ_LENGTH;
    document.getElementById("quizModalTitle").textContent = "第 " + station.level + " 关 · " + station.name;
    document.getElementById("quizProgressText").textContent = "第 " + station.level + " 关 · " + Math.min(answered + 1, target) + " / " + target + " 题";
    document.getElementById("quizProgressBar").style.width = (answered / Math.max(target, 1)) * 100 + "%";
    var diffLabels = ["入门", "基础", "进阶", "熟练", "挑战"];
  ...
  function renderQuestion() {
      ...
      html += q.options.map(function (opt, i) {
        ...
        if (saved && saved.status !== undefined) {
          if (selected.indexOf(i) >= 0) {
            cls += saved.correct ? " correct" : " wrong";
          }
        }
    ...
    if (saved && saved.status !== undefined) {
      html +=
        ...
        "<span>" + (saved.correct ? "回答正确" : saved.result === "partial" ? "部分正确" : "回答错误") + "</span></div>" +
        '<p class="npc-feedback-line">' + esc(saved.reaction || "") + "</p>" +
        "<span>" + esc(saved.explanation || q.explanation || "") + "</span>" +
        "</div>";
    }
  ...
  function quizAnswerValue(q, indexes, textValue) {
    if (q.type === "fill") return textValue || "";
    if (q.type === "single" || q.type === "judge" || q.type === "scene") {
      return Array.isArray(indexes) && indexes.length ? indexes[0] : null;
    }
    return Array.isArray(indexes) ? indexes : [];
  }

  async function submitAnswer(q, indexes, textValue) {
    if (!state.api.session || state.quiz.answers[q.id]) return;
    var answer = quizAnswerValue(q, indexes, textValue);
    if (answer === null || answer === "") {
      showToast("请先完成作答");
      return;
    }
    var previousLevelId = state.api.session.currentLevel ? state.api.session.currentLevel.id : null;
    try {
      var response = await apiRequest("/api/sessions/" + state.api.session.id + "/answers", {
        method: "POST",
        body: {
          questionId: q.id,
          answer: answer,
          elapsedSeconds: state.quiz.questionStartedAt ? Math.max(1, Math.round((Date.now() - state.quiz.questionStartedAt) / 1000)) : 1
        }
      });
      state.quiz.answers[q.id] = {
        ...
        value: answer,
        selected: Array.isArray(indexes) ? indexes.slice() : [],
        reaction: response.npcReaction || "",
        explanation: response.explanation || "",
        score: response.score
      };
      state.quiz.diff = Number(response.session.difficulty) || state.quiz.diff;
      applySession(response.session);
      state.quiz.lastLevelId = previousLevelId;
      saveQuizSession();
      renderQuestion();
      if (response.flags && response.flags.length) {
        showToast("系统检测到 " + response.flags.length + " 项异常记录");
      }
    } catch (error) {
  ...
  function nextQuestion() {
    var lastAnsweredId = state.quiz.questionIds[state.quiz.current];
    var saved = state.quiz.answers[lastAnsweredId];
    if (!saved) {
      var q = state.api.currentQuestion;
      if (!q) {
        finishQuiz();
        return;
      }
      if (q.type === "fill") {
        ...
        submitAnswer(q, [], (input && input.value.trim()) || "");
      } else if (q.type === "sort") {
        var order = (state.quiz.selected[q.id] || []).slice();
        if (order.length !== q.options.length) order = q.options.map(function (_, i) { return i; });
        submitAnswer(q, order);
      } else if (q.type === "multi") {
        var selected = state.quiz.selected[q.id] || [];
        if (!selected.length) showToast("请先选择答案");
        else submitAnswer(q, selected);
      } else {
        showToast("请先选择答案");
      }
      return;
    ...
    var nextTask = state.api.currentQuestion && state.api.currentQuestion.type === "practical" ? "practical" : state.api.currentQuestion && state.api.currentQuestion.type === "dialogue" ? "dialogue" : null;
    if (nextTask || (state.quiz.lastLevelId && state.quiz.currentLevel !== state.quiz.lastLevelId)) {
      openLevelTransition(nextTask);
      return;
    }
    if (state.api.currentQuestion) {
      state.quiz.current = 0;
      state.quiz.questionIds = [state.api.currentQuestion.id];
      state.quiz.questionStartedAt = Date.now();
      startQuizTimer();
  ...
  function openLevelTransition(nextTask) {
    stopQuizTimer();
    ...
    closeModal("quizModal");
    var station = currentLevelStation();
    if (nextTask === "practical") {
      openResult("创客工坊 · 实操任务", "客观题阶段已完成，接下来进入 AI 终端，请至少迭代两次提示词后提交产物。", 2, "practical");
    } else if (nextTask === "dialogue") {
      openResult("协同空间站 · 对话式测评", "本关客观题已完成，接下来与星核 AI 完成 2-4 轮危机对话并提交。", 2, "dialogue");
    } else {
      openResult("第 " + station.level + " 关完成", station.name + " 的客观题阶段已提交，关卡状态已同步到后端。", station.stars || 2, "stations");
    }
  }

  function finishAssessmentIfNeeded() {
    if (state.api.session && state.api.session.status === "levels_completed") return finishQuiz();
  ...
  async function finishQuiz() {
    ...
    state.quiz.started = false;
    closeModal("quizModal");
    if (!state.api.session) {
      showToast("当前没有可完成的测评会话");
      return;
    }
    try {
      var report = null;
      if (state.api.session.status !== "levels_completed") {
        var latest = await apiRequest("/api/sessions/" + state.api.session.id);
        applySession(latest.session);
      }
      if (state.api.session.status === "levels_completed") {
        report = await apiRequest("/api/sessions/" + state.api.session.id + "/finish", { method: "POST" });
        state.api.latestReport = report;
        await refreshHistory();
      }
      ...
      renderRetestGoal();
      clearQuizSession();
      openResult("智核觉醒 · 完整通关", "五个剧情区域全部完成，本轮报告已由后端生成，学习分析已同步刷新。", report ? report.stars : 15, "analysis");
      var ending = document.getElementById("npcEnding");
      ending.hidden = false;
      document.getElementById("npcEndingText").textContent = NPC_ENDING;
    } catch (error) {
      showToast(error.message || "报告生成失败，请稍后重试");
    }
  }
  ...
  function chatScript() {
    if (!state.api.currentQuestion || state.api.currentQuestion.type !== "dialogue") return [];
    return state.api.currentQuestion.script || [];
  }

  function startChat() {
    var script = chatScript();
    if (!script.length) {
      showToast("当前测评尚未到达对话关卡，请先完成前置关卡");
      switchView("stations");
      return;
    }
    var body = document.getElementById("chatBody");
    ...
    state.chat.hits = 0;
    state.chat.messages = [];
    state.chat.scriptLength = script.length;
    state.chat.submitted = false;
    state.chat.startedAt = Date.now();
    document.getElementById("roundBadge").textContent = "第 1 轮 · 共 " + script.length + " 轮";
    addBubble("npc", script[0].ask);
    renderQuick(script[0].quickReplies || []);
  }

  function chatReplyFor(round, text) {
    var script = chatScript();
    var item = script[round] || {};
    var keywords = ["依据", "风险", "实时", "数据", "备选", "验证", "人工", "决策", "安全", "边界", "方案", "复核"];
    var hit = keywords.filter(function (k) {
      return String(text || "").indexOf(k) >= 0;
    });
    if (hit.length) {
      state.chat.hits += 1;
      return "关键词命中：“" + hit[0] + "”。你正在围绕依据、风险或决策边界追问，星核会记录你的表达。";
    }
    return "已记录。请继续补充目标、依据或风险边界，星核会据此评估人机协同水平。";
  }

  async function submitDialogue() {
    var question = state.api.currentQuestion;
    if (!question || question.type !== "dialogue") return;
    try {
      var response = await apiRequest("/api/sessions/" + state.api.session.id + "/answers", {
        method: "POST",
        body: {
          ...
          elapsedSeconds: state.chat.startedAt ? Math.max(1, Math.round((Date.now() - state.chat.startedAt) / 1000)) : 60
        }
      });
      state.chat.submitted = true;
      applySession(response.session);
      var stars = response.score >= 85 ? 3 : response.score >= 65 ? 2 : 1;
      if (response.session.status === "levels_completed") {
        finishQuiz();
        return;
      }
      var nextTask = response.session.currentQuestion && response.session.currentQuestion.type === "practical" ? "practical" : null;
      openResult("对话式测评完成", "星核已记录你的 " + state.chat.messages.length + " 轮对话，后端完成关键词计分，本关得分 " + response.score + " 分。", stars, nextTask || "stations");
    } catch (error) {
      state.chat.busy = false;
      showToast(error.message || "对话提交失败，请重试");
    }
  }
  ...
  function sendChat(text) {
    ...
    setTimeout(function () {
      ...
      state.chat.round += 1;
      state.chat.messages.push(text.trim());
      var script = chatScript();
      if (state.chat.round < script.length) {
        addBubble("npc", script[state.chat.round].ask);
        renderQuick(script[state.chat.round].quickReplies || []);
        document.getElementById("roundBadge").textContent = "第 " + (state.chat.round + 1) + " 轮 · 共 " + script.length + " 轮";
        state.chat.busy = false;
      } else {
  ...
  async function requestGeneratedOutput(prompt) {
    ...
    var response = await fetch("/api/ai/generate", {
      ...
      body: JSON.stringify(Object.assign(
        { prompt: prompt },
        state.api.session && state.api.currentQuestion && state.api.currentQuestion.type === "practical"
          ? { sessionId: state.api.session.id, questionId: state.api.currentQuestion.id }
          : {}
      ))
    });
  ...
  async function submitPractical() {
    ...
    if (state.api.session && state.api.currentQuestion && state.api.currentQuestion.type === "practical") {
      try {
        var result = await apiRequest("/api/sessions/" + state.api.session.id + "/practical/submit", { method: "POST" });
        state.practical.submitted = true;
        state.practical.score = Number(result.score) || 0;
        state.practical.stars = Number(result.stars) || 1;
        state.practical.synced = true;
        savePractical();
        ...
        var resultMeta = document.querySelector("#practicalResult .result-head strong + span");
        if (resultMeta) resultMeta.textContent = "评分 " + state.practical.score + " · " + state.practical.stars + " 颗星";
        showToast("产物已提交，创客工坊得分 " + state.practical.score + " 分");
        if (result.session.status === "levels_completed") {
          finishQuiz();
          return;
        }
        openResult("实操任务完成", "你的产物已提交，本轮迭代 " + state.practical.runs + " 次，后端评分 " + state.practical.score + " 分。", state.practical.stars, "stations");
      } catch (error) {
        showToast(error.message || "实操提交失败，请重试");
      }
      return;
    }
    state.practical.submitted = true;
    ...
    savePractical();
    var standaloneBox = document.getElementById("practicalResult");
    standaloneBox.hidden = false;
    document.getElementById("practicalResultText").textContent = state.practical.lastOutput.split("\n\n质量评估")[0];
  ...
  function checkResumeSession() {
    if (!state.api.session || state.api.session.status !== "active" || !state.api.currentQuestion) {
      hideResumeBanner();
      return;
    }
    var station = currentLevelStation();
    ...
    state.quiz.currentLevel = state.api.session.currentLevel ? state.api.session.currentLevel.id : station.id;
    state.quiz.questionIds = [state.api.currentQuestion.id];
    state.quiz.answers = {};
    state.quiz.selected = {};
    state.quiz.seconds = Math.max(30, Math.round((Number(state.api.currentQuestion.estimatedSeconds) || 90) / 3));
    state.quiz.diff = Number(state.api.session.difficulty) || state.quiz.diff;
    state.quiz.started = true;
    var banner = document.getElementById("resumeBanner");
    banner.hidden = false;
    document.getElementById("resumeText").textContent = "上次进行到第 " + station.level + " 关 · " + station.name + "，可继续作答，进度已同步到后端。";
  }

  function resumeQuiz() {
    var station = currentLevelStation();
    if (!state.api.session || !state.api.currentQuestion || !station) {
      hideResumeBanner();
      return;
    }
    if (state.api.currentQuestion.type === "practical") {
  ...
  async function saveProfile() {
    ...
    try {
      await apiRequest("/api/students/me/profile", {
        method: "PUT",
        body: { nickname: state.nickname, avatarId: state.avatarId }
      });
      localStorage.setItem("ai-student-nickname", state.nickname);
      localStorage.setItem("ai-student-avatar", state.avatarId);
    } catch (error) {
  ...
  function bindEvents() {
    ...
    document.getElementById("resumeDiscard").addEventListener("click", function () {
      apiRequest("/api/sessions/current", { method: "DELETE" }).catch(function () {});
      clearQuizSession();
      hideResumeBanner();
    ...
    document.getElementById("clearHistoryBtn").addEventListener("click", function () {
      showToast("测评历史由后端统一管理，如需删除请联系管理员");
    });
    ...
    document.addEventListener("visibilitychange", function () {
      ...
      if (document.hidden && state.api.session && state.quiz.started && quizEl.classList.contains("open") && content && !content.hidden) {
        apiRequest("/api/sessions/" + state.api.session.id + "/anti-cheat", { method: "PATCH", body: { pageHidden: true } }).catch(function () {});
